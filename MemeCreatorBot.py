...
class Meme:
    def __init__(self, areas, fontname, font_colour):
        self.areas = areas
...
expanding_brain = Meme({(422, 296): (0, 0), (420, 301): (2, 303), (417, 269): (2, 611), (419, 304): (2, 895)}, 'impact.ttf', (0, 0, 0))
...
is_this_a_pigeon = Meme({(326, 110): (28, 65), (217, 84): (416, 91), (568, 90): (33, 478)}, 'impact.ttf', (255, 255, 255))
distracted_boyfriend = Meme({(161, 88): (97, 271), (150, 62): (309, 163), (151, 75): (449, 222)}, 'impact.ttf', (0, 0, 0))
croatia_bosnia_border = Meme({(418, 157): (305, 128), (449, 137): (360, 370), (528, 130): (40, 623)}, 'impact.ttf', (0, 0, 0))
left_exit_12_off_ramp = Meme({(262, 181): (101, 88), (276, 182): (369, 88), (427, 143): (195, 501)}, 'impact.ttf', (255, 255, 255))
hard_to_swallow_pills = Meme({(278, 216): (135, 558)}, 'impact.ttf', (0, 0, 0))
trump_presenting = Meme({(289, 96): (2, 251), (308, 100): (133, 146), (409, 199): (383, 274)}, 'impact.ttf', (255, 255, 255))
double_d_facts_book = Meme({(264, 130): (20, 494)}, 'impact.ttf', (0, 0, 0))
water_gun = Meme({(383, 112): (106, 343), (295, 214): (500, 369)}, 'impact.ttf', (0, 0, 0))
man_bear_fish = Meme({(561, 116): (103, 388), (333, 114): (219, 219), (232, 253): (560, 172)}, 'impact.ttf', (255, 255, 255))
upvotes = Meme({(177, 45): (8, 392)}, 'impact.ttf', (0, 0, 0))
who_killed_hannibal = Meme({(319, 275): (513, 178), (307, 234): (83, 229), (930, 118): (15, 958)}, 'impact.ttf', (255, 255, 255))
...
def create_meme(message, curr_meme, meme_texts):
    ...
    font_type = ImageFont.truetype(Memes[curr_meme].fontname, fontsize)
    for WH, position in Memes[curr_meme].areas.items():
        ...
        if draw.textsize(text, font=font_type)[0] < width:
            ...
            for i in words:
                ...
                if words.index(i) == len(words) - 1:
                    modifiedtext += currstr
                while draw.textsize(modifiedtext, font=font_type)[1] > height or draw.textsize(modifiedtext, font=font_type)[0] > width:
                    fontsize -= 1
                    font_type = ImageFont.truetype('impact.ttf', fontsize)
        datas = im.getdata()
        new_data = []
        for item in datas:
        ...
        if Memes[curr_meme].font_colour == (255, 255, 255):
            draw.text(((width - W) / 2 - 1, (height - H) / 2 - 1), modifiedtext, font=font_type, fill=(0, 0, 0), spacing=3, align='center')
            draw.text(((width - W) / 2 + 1, (height - H) / 2 - 1), modifiedtext, font=font_type, fill=(0, 0, 0), spacing=3, align='center')
            draw.text(((width - W) / 2 - 1, (height - H) / 2 + 1), modifiedtext, font=font_type, fill=(0, 0, 0), spacing=3, align='center')
            draw.text(((width - W) / 2 + 1, (height - H) / 2 + 1), modifiedtext, font=font_type, fill=(0, 0, 0), spacing=3, align='center')
